import time
import argparse
from src.audio_stream_analyzer import Audio_Stream_Analyzer
...
def parse_args():
    ...
    parser.add_argument('--n_frequency_bins', type=int, default=400, dest='frequency_bins',
                        help='The audio features are grouped in bins')
...
def convert_window_ratio(window_ratio):
    if '/' in window_ratio:
        ...
        try:
            ...
            raise ValueError(
                'window_ratio should be in the format: float/float')
        return float_ratio
...
def main():
    ...
    ear = Audio_Stream_Analyzer(
        device=args.device,
        rate=None,               # Audio samplerate, None uses the default source settings
        audio_window_size_ms=60,    # Window size used for the audio transform
        updates_per_second=1000,  # How often to read the audio stream for new data
        smoothing_length_ms=50,    # Apply some temporal smoothing to reduce noisy features
        n_frequency_bins=args.frequency_bins,  # The audio features are grouped in bins
        visualize=1,               # Visualize the audio features with PyGame
